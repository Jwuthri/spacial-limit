...
class ApiService {
  ...
  // Analyze image with overlay (returns image with boxes drawn on it)
  async analyzeImageWithOverlay(request: AnalysisRequest): Promise<ApiResponse & { overlay_image?: string }> {
    try {
      ...
      const response_data: AxiosResponse<ApiResponse & { overlay_image?: string }> = await this.api.post(
        '/analyze-with-overlay',
        formData,
        {
          headers: {
      ...
      console.error('Error analyzing image with overlay:', error);
      throw this.handleError(error);
    }
  }

  // Analyze image (original endpoint)
  async analyzeImage(request: AnalysisRequest): Promise<ApiResponse> {
    try {
      ...
      formData.append('segmentation_language', request.segmentation_language);
      formData.append('temperature', request.temperature.toString());
      // Don't skip resize - let backend handle resizing for now
      // formData.append('skip_resize', 'true'); 
